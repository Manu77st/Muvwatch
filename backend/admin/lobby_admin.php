...
$posterUploadsUrl = '/Muvwatch/uploads/posters/';
...
$sql = "SELECT f.id_funcion, f.id_sala, f.fecha_funcion, f.precio, f.descuento,
          p.id_pelicula, p.nombre, p.sipnosis, p.genero, p.reparto, p.director, p.clasificacion, p.poster_url          
            FROM tbl_funcion f INNER JOIN tbl_pelicula p ON p.id_pelicula = f.id_pelicula
...
<body>
    <nav class="navbar">
        <div class="container-navbar">
            <div class="navbar-left">
                <a href="/Muvwatch/backend/admin/lobby_admin.php" class="logo-link">
                    <img id="logo-nav" src="/Muvwatch/images/Logo.svg" alt="Logo_muvwatch">
                </a>
            </div>
            <div class="navbar-center">
                <!-- Filtro eliminado para interfaz admin limpia -->
            </div>
            <div class="navbar-right">
                <div class="cliente-box" id="clienteBox">
                    ...
                    <div class="menu-cliente" id="menuCliente">
                        <a href="#">Perfil</a>
                        <a href="/Muvwatch/backend/logout.php">Cerrar sesión</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="menu-secundario">
        <nav>
            <a href="/Muvwatch/backend/admin/lobby_admin.php" class="enlace-a" style="border-bottom-color: #3b7ba6; background: #f5f5f5">Inicio</a>
            <a href="/Muvwatch/src/Reportes-ventas.html">Reportes</a>
            <a href="/Muvwatch/backend/admin/Consultar-clientes-VIP.php">Clientes VIP</a>
        </nav>
    </div>
    ...
    <main class="main-content">
        <div class="movies-grid">
            <div class="movie-card add-card" onclick="window.location.href='/Muvwatch/src/añadir-funcion.html'">
            ...
            <?php else: ?>
                <?php foreach ($funciones as $funcion): ?>
                    ...
                    // 1. Si hay poster_url en BD, úsalo
                    if (!empty($funcion['poster_url'])) {
                        $poster = $funcion['poster_url'];
                    } elseif ($idPelicula > 0) {
                        // 2. Buscar en uploads/posters
                        $patternUploads = rtrim($posterUploadsDir, '/\\') . '/pelicula_' . $idPelicula . '.*';
                        $posterCandidatesUploads = glob($patternUploads);
                        if (!empty($posterCandidatesUploads)) {
                            $poster = $posterUploadsUrl . basename($posterCandidatesUploads[0]);
                        } else {
                            // 3. Buscar en images/Peliculas_Cartelera
                            $imageDir = realpath(__DIR__ . '/../../images/Peliculas_Cartelera');
                            $patternImages = $imageDir . '/*' . $titulo . '*.*';
                            $posterCandidatesImages = glob($patternImages);
                            if (!empty($posterCandidatesImages)) {
                                // Usar ruta relativa para el navegador
                                $poster = '/Muvwatch/images/Peliculas_Cartelera/' . basename($posterCandidatesImages[0]);
                            }
                        }
                    }
                    ?>
                    <div class="movie-card"
                        ...
                        <div class="movie-actions">
                            ...
                            <form class="delete-form" action="/Muvwatch/backend/admin/eliminar_funcion.php" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar la función <?= $titulo; ?>?');">
    ...
    <script>
        ...
        if (modalDisableButton) {
            modalDisableButton.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch('/Muvwatch/backend/admin/deshabilitar_pelicula.php', {
                        method: 'POST',
                        headers: {
        ...
        document.querySelectorAll('.btn-enable').forEach((button) => {
            button.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch('/Muvwatch/backend/admin/habilitar_pelicula.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
                    ...
                    window.location.href = '/Muvwatch/backend/admin/lobby_admin.php?msg=pelicula_habilitada';
                } catch (error) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            clienteBox.addEventListener('click', function () {
                menuCliente.classList.toggle('show');
            });

            document.addEventListener('click', function (event) {
                if (!clienteBox.contains(event.target)) {
...
<script src="/Muvwatch/scripts/menu-cliente.js"></script>
<script src="/Muvwatch/scripts/admin-search.js"></script>
</body>
