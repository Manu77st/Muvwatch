...
-- --------------------------------------------------------
-- (tbl_bloquesillas) ----> ELIMINADA: no se crea ni se insertan filas
--  Nota: los datos de bloques originales han sido usados para asignar id_sala en tbl_sillas.
-- --------------------------------------------------------
...
-- Aquí migramos los datos originales:
-- Original inserts were:
-- (1, 1, 'A', 'Standard', 0),
-- (2, 1, 'A', 'Discapacitados', 1);
-- We map: id_sala from the original bloque (bloque 1 -> sala 1),
-- columna assigned sequentially per fila: 1,2
-- disponible -> activa (0/1)
INSERT INTO `tbl_sillas` (`id_silla`, `id_sala`, `fila`, `columna`, `tipo`, `activa`) VALUES
(1, 1, 'A', 1, 'Standard', 0),
(2, 1, 'A', 2, 'Discapacitado', 1);
...
-- (tbl_bloquesillas) ----> eliminado: no se crean índices para esta tabla
...
-- Indices de la tabla `tbl_sillas` (actualizado: index sobre id_sala)
...
-- (tbl_bloquesillas) eliminado: no AUTO_INCREMENT
...
-- detalles_venta -> tbl_ventas, tbl_sillas
ALTER TABLE `detalles_venta`
...
-- tbl_funcion -> tbl_pelicula, tbl_salas
ALTER TABLE `tbl_funcion`
...
-- tbl_reservas -> tbl_clientes, tbl_funcion
ALTER TABLE `tbl_reservas`
...
-- tbl_sillas -> tbl_salas  (NUEVA FK, antes apuntaba a tbl_bloquesillas)
ALTER TABLE `tbl_sillas`
  ADD CONSTRAINT `tbl_sillas_ibfk_1` FOREIGN KEY (`id_sala`) REFERENCES `tbl_salas` (`id_sala`) ON DELETE CASCADE;

-- tbl_ventas -> tbl_clientes, tbl_cajeros, tbl_funcion
ALTER TABLE `tbl_ventas`
